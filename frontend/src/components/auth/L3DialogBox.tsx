...
const handleUndergraduateChange = (
  ...
  try {
    // Dynamically validate the single field using Joi
    const fieldSchema = UndergraduateSchema.extract(name);
    const { error } = fieldSchema.validate(normalizedValue);

    // Update errors: clear if valid, set message if invalid
    setUndergraduateFormErrors((prev) => ({
      ...prev,
      [name]: error ? error.message : "",
    }));
  } catch (err) {
    // If the field name does not exist in schema
    console.warn(`No validation schema found for ${name}`);
  }
};
...
const handleUndergraduateSubmit = async (e: FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    // ✅ Convert Yes/No to booleans where applicable
    const undergraduateFormDataWithInstitute = {
      ...undergraduateFormData,
      instituteType: savedFormData.instituteType,
    ...
    // ✅ API call
    const response = await institutionDetailsAPI.createInstitutionDetails(
      undergraduateFormDataWithInstitute
    );
    ...
    if (response.success) {
      ...
      alert(response.message || "Failed to save UG/PG details. Please try again.");
    }
  } catch (error) {
    console.error("Error saving UG/PG details:", error);
    alert("Failed to save UG/PG details. Please try again.");
  } finally {
    setIsLoading(false);
  }
};
...
//   e.preventDefault();
//   console.log("In handle submit of ugpg")
