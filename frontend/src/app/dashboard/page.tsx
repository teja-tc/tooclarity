...
function DashboardPage() {
  ...
  const generateCustomers = useCallback(() => {
    ...
    const statuses = ["Requested for callback", "Requested for demo"];
    const programs = [
  ...
  const fetchDataWithTrends = useCallback(
    async (range: "weekly" | "monthly" | "yearly") => {
      try {
        ...
        if ((comparisonsData as any)?.success) {
          ...
          setStats((prev) => ({
            ...
            courseComparisonsTrend: data.trend || {
              value: 0,
              isPositive: true,
            },
          }));
  ...
  const fetchRecentAndChart = useCallback(async () => {
    try {
      ...
      if (
        (recentEnquiries as any)?.success &&
        Array.isArray((recentEnquiries as any).data?.enquiries)
      ) {
        const enquiries = (recentEnquiries as any).data.enquiries;
        const mapped: CustomerItem[] = enquiries.map(
          (enquiry: any, idx: number) => ({
            date: new Date(
              enquiry.createdAt || Date.now() - idx * 86400000
      ...
      if (
        (seriesRes as any)?.success &&
        Array.isArray((seriesRes as any).data?.series)
      ) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        s.on("courseViewsUpdated", async () => {
          try {
            ...
            if (
              (series as any)?.success &&
              Array.isArray((series as any).data?.series)
            ) {
              const arr = new Array(12).fill(0);
              (series as any).data.series.forEach(
                (count: number, idx: number) => {
                  if (idx >= 0 && idx < 12) arr[idx] = count || 0;
                }
              );
              setChartValues(arr);
            }
        ...
        s.on("comparisonsUpdated", async () => {
          try {
            const latest = await metricsAPI.getOwnerByRange(
              "comparisons",
              filters.timeRange
            );
            if ((latest as any)?.success) {
    ...
    return () => {
      try {
        s?.off("courseViewsUpdated");
        s?.off("comparisonsUpdated");
        s?.off("enquiryCreated");
        s?.off("ownerTotalLeads");
      } catch {}
    };
  ...
  const handleFilterChange = async (newFilters: FilterState) => {
    const normalized: FilterState = {
      ...newFilters,
      timeRange: newFilters.timeRange || "weekly",
    };
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="-mx-2 sm:-mx-4 lg:-mx-6">
        <CourseReachChart
